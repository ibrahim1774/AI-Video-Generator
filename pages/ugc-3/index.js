...
export default function Ugc3Page() {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      // UGC-3 pre-subscribe landing: skip the upload-time NSFW/minor
      // pre-screen so the first page is frictionless. Generation-time
      // moderation in /api/ugc-animate + /api/ugc-image stays ON, so
      // nothing harmful is ever actually generated.
  ...
  return (
    <>
      ...
      <main className={styles.page} style={{ paddingTop: 8 }}>
        <div className={`${styles.hero} fade-up`} style={{ marginBottom: 6 }}>
          <h1
