...
/*
 * /ugc-2/app — gated UGC creator surface for the pay-first flow.
 *
 * Mirrors the creator UI from /ugc but only renders for users with
 * an active subscription. A separate FEATURE namespace ('ugc-2')
 * keeps in-progress local-storage state isolated from /ugc so the
 * two surfaces don't stomp each other's job/story rehydration.
 *
 * Auth gate: middleware redirects anonymous users to /sign-in.
 * Sub gate: this component redirects to /ugc-2 if entitlement is not
 * an active monthly/yearly/admin tier.
 */
const FEATURE = 'ugc-2';
const MAX_SCENES = 5;
...
export default function Ugc2AppPage() {
  ...
  // Sub gate: middleware already redirects anon users to /sign-in. Once
  // auth resolves, fetch entitlement and bounce to the landing page if
  // the user does not have an active paid plan.
  useEffect(() => {
    if (!authLoaded) return;
    if (!authUser) {
      // Middleware should have redirected already, but if not (e.g. the
      // session cookie was missing client-side), send them to landing.
      router.replace('/ugc-2');
      return;
    }
    let cancelled = false;
    (async () => {
  ...
  }, [authLoaded, authUser, fetchEntitlement, router]);

  // Resume in-flight jobs + rehydrate story once the gate passes.
  useEffect(() => {
    ...
    return (
      <main className={styles.page}>
        ...
        <Paywall
          ...
          onTrialStarted={() => {
            fetchEntitlement();
            setStep(storyScenes.length > 0 ? 'result' : 'choose');
          }}
        />
        <div style={{ textAlign: 'center', marginTop: 16 }}>
          <button
            type="button"
            onClick={() =>
              setStep(storyScenes.length > 0 ? 'result' : imageUrl ? 'animate' : 'choose')
            }
            style={{
              background: 'transparent',
              border: '1px solid rgba(255,255,255,0.15)',
  ...
  if (step === 'animate' && effectiveStartImage) {
    ...
    return (
      <>
        <Head><title>UGC-2 Creator — Haelabs</title></Head>
        <main className={styles.page}>
          <div className={styles.hero}>
