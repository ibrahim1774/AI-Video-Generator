...
export async function createStoryboardPrediction({
  ...
  const multi = scenes.map((s) => {
    const promptText = (s.prompt || '').trim() || 'moves naturally';
    // Follow the Replicate example pattern — place the reference tag
    // as the subject of the sentence, not as a prefix clause.
    return {
