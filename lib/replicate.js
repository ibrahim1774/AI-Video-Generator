...
/**
 * Kick off a Kling v2.1 image-to-video prediction.
 *
 * @param {object} args
 * @param {string} args.imageUrl - public URL of the starting image
 * @param {string} args.prompt - motion / scene prompt
 * @param {5|10} [args.duration=5] - output length in seconds
...
/**
 * Generate a fresh character image with Nano Banana Pro from a text
 * prompt only (no source frame). Returns the resulting image URL.
 */
export async function createBananaImage({ prompt }) {
