...
async function readPaidEntitlement(customerId) {
  ...
  const planRaw = planFromPrice(price);
  if (!planRaw) return null;

  const customer = await stripe().customers.retrieve(customerId);
  const md = customer && !customer.deleted ? customer.metadata || {} : {};

  // Paid 1-day trial: customer is on the $1/day price, but UX-wise
  // they're a trialing yearly subscriber. Return the same shape as
  // the legacy free-trial branch (2 credits + topups, status:
  // trialing, tier: yearly). When Stripe transitions the schedule to
  // the yearly price, the next entitlement read sees plan === 'yearly'
  // and the active branch grants the full 48 credits via the rollover.
  if (planRaw === 'trial-day') {
    ...
    return {
      ...
      customerId,
      trialEnd: sub.current_period_end
