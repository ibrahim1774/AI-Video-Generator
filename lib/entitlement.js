...
async function readPaidEntitlement(customerId) {
  ...
  // During the 1-day Stripe trial, hard-cap at 1 swap regardless of
  // the plan's normal allotment. After trial converts to 'active',
  // the full plan cap applies.
  const baseCap = CAPS[plan];
  const cap = sub.status === 'trialing' ? 1 : baseCap;
  return {
    tier: plan,
